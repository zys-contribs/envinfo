...
function getyarnPackages(packages, options) {
  utils.log('trace', 'getnpmPackages');
  if (!options) options = {};
...
function getyarnGlobalPackages(packages, options) {
  utils.log('trace', 'getyarnGlobalPackages', packages);
      ...
      .then(
        prefix =>
          new Promise((resolve, reject) =>
            glob(
              // sub packageGlob in to only get globbed packages if not null
              path.join(prefix, 'node_modules', packageGlob || '{*,@*/*}', 'package.json'),
              (err, files) => {
                if (!err) resolve(files);
                reject(err);
              }
...
module.exports = {
  ...
  getyarnPackages: getyarnPackages,
  getyarnGlobalPackages: getyarnGlobalPackages,
};
